...
public class LocalStorage implements Storage {
                        ...
                        for (ViolationX violation : violations) {
                            try {
                                ...
                                Query.prepare("INSERT INTO `ALERTS` (`UUID`, `MODULE`, `VL`, `TIME`, `EXTRA`, `COORDS`, `WORLD`, `PING`, `TPS`) VALUES (?,?,?,?,?,?,?,?,?)")
                                        ...
                                        .append(violation.time).append(violation.data)
                                        .append(violation.location).append(violation.world)
                                        .append(violation.ping).append(violation.TPS).execute();


                            } catch (Exception e) {

                                if(e.getMessage().contains("[SQLITE_ERROR] SQL error or missing database (table ALERTS has no column named WORLD)")) {

                                    File source = new File(Karhu.getInstance().getDataFolder().getAbsolutePath() + "/" + "database.sqlite");
                                    File dest = new File(Karhu.getInstance().getDataFolder().getAbsolutePath() + "/" + "database-old.sqlite");
    ...
    public void setAlerts(String uuid, int status) {
        SQLite.use();
        Query.prepare("REPLACE INTO `ALERTSTATUS` (`UUID`, `STATUS`) VALUES (?,?)")
